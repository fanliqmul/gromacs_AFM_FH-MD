...
int fhmd_init(matrix box, int N_atoms, real mass[], rvec x[], double dt_md, t_grpopts *opts ,gmx_mtop_t *mtop, t_commrec *cr,gmx_fhmd_global_stat *g_fhmd_s ,FHMD *fh)
{
	...
	if (fh->S_function == moving_sphere)
	{
		...
		//strcpy(fh->prot_name[0],fh->prot_name_1);
		//strcpy(fh->prot_name[1],fh->prot_name_2);

		split_string(fh->prot_name,fh->tem,fh->prot_num);
    ...
    if(MASTER(cr))
    {
	 ...
	 if (fh->S_function == AFM)
	 {
		...
		{
			//if(MASTER(cr))
				printf(MAKE_RED "Inconsistency between flow_type in input and flow velocity field" RESET_COLOR "\n");
			exit(5);
		}
	}
    ...
    if(PAR(cr)) gmx_bcast(sizeof(fh->arr), fh->arr, cr);

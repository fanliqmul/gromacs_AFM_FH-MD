...
/*void fhmd_add_flow_velocity(FHMD *fh)
{
	FH_arrays *arr = fh->arr;
	ivec ind;
	 ...
	 for(int k = fh->N_shift[2]; k < (fh->N_md[2] + fh->N_shift[2]); k++)
	 {
		 for(int j = fh->N_shift[1]; j < (fh->N_md[1] + fh->N_shift[1]); j++)
	     {
			 for(int i = fh->N_shift[0]; i < (fh->N_md[0] + fh->N_shift[0]); i++)
			 {
				 ...
				 for(int d = 0; d < DIM; d++)
				 {
					 arr[C].u_fh[d] +=	arr[C].u_fh_flow[d];
#ifdef FLOW_VELOCITY_DEBUG
					 arr[C].u_fh[d]  = 0*arr[C].u_fh[d] + arr[C].u_fh_flow[d];
					 if(fh->Smax !=1 )
					 {
						 printf(MAKE_RED "FHMD error: The fh velocity is equal to flow velocity,it happens only in debug mode and "
								 "Smax should be 1" RESET_COLOR "\n");
						 exit(0);
					 }
#endif
				 }

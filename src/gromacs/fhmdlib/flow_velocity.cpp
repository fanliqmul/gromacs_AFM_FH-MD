...
void compute_vx_instant_mpi (FHMD *fh, rvec x[], rvec v[], real mass[], int N_atoms, t_commrec *cr)
{
	 ...
	 /*sprintf(fname, "thread_%d.txt", cr->nodeid);
	 sprintf(fname2,"bin_val_thread_%d.txt",cr->nodeid);
	 fp = fopen(fname, "w");
	 fp_value = fopen(fname2,"w");
	 if (PAR(cr))
	 {
	 	 //printf("thread %d     number of atoms %d \n",cr->nodeid,N);
	       fprintf(fp,"step %d thread_%d number of atoms %d  \n",fh->step_MD,cr->nodeid,N);
	       fprintf(fp_value,"vx    vx_m\n");
	 }*/
...
void split_string (char **prot_name,char const *tem,int prot_num)
{
	int i,j,ctr;
	 ...
	 for(i=0;i<=(strlen(tem));i++)
	{
		...
		{
			prot_name[ctr][j]=tem[i];
			j++;
		}

		if (ctr < prot_num) continue;
		else break;
	}
}

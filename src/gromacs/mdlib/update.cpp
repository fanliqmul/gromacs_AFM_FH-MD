...
/*static void calc_FHMD_ke_part_normal (rvec v[], t_grpopts *opts, t_mdatoms *md,
        						      gmx_ekindata_t *ekind, t_nrnb *nrnb, gmx_bool bEkinAveVel,FHMD *fh)
{
	...
	if(g>1)
	{
		printf(MAKE_RED "FHMD error: More than one temperature coupling group,please use System in mdp file");
	    exit (3);
	}

	copy the KE at last time in ekinh to ekinh_old, and clear the ekinh
	for(i = 0; i< fh->nzbin; i++)
	{
		copy_mat(ke_data[i].ekinh, ke_data[i].ekinh_old);
		if (bEkinAveVel)
		{
			printf(MAKE_RED "FHMD error: The velocity verlet integrator is not supported\n");
		}
   	   ...
   	   gmx_ekindata_t *ekind, t_nrnb *nrnb, gmx_bool bEkinAveVel,FHMD *fh)
{
	if (ekind->cosacc.cos_accel == 0)
	{
		calc_FHMD_ke_part_normal(state->v, opts, md, ekind, nrnb, bEkinAveVel,fh);
	}

	else
	{
		printf(MAKE_RED "FHMD: NEMD is not supported\n");
	}
}*/
